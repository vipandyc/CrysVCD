...
from .utils.data_utils import radius_graph_pbc, repeat_blocks
...
class CSPNet(nn.Module):
    ...
    def __init__(
        self,
        hidden_dim = 512,
        latent_dim = 512, # 512=256(time)+256(properties)
        num_layers = 4,
        max_atoms = 100,
        act_fn = 'silu',
        dis_emb = 'sin',
    ...
    def forward(self, t, prop_emb, atom_types, frac_coords, lattices, num_atoms, node2graph):
        edges, frac_diff = self.gen_edges(num_atoms, frac_coords, lattices, node2graph)

        prop_per_atom = prop_emb[node2graph] # assign correctly to different graphs.
        ...
        # node_features.shape: total_atoms_per_batch*512
        node_features = torch.cat([node_features, t_per_atom, prop_per_atom], dim=1)
        # node_features.shape: total_atoms_per_batch*(512+256+256)
